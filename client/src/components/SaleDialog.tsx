...
import { insertSaleSchema, type WorkOrder, type Customer, type Service, type InventoryItem } from "@shared/schema";
...
export default function SaleDialog({ isOpen, onClose, workOrder, customer, services }: SaleDialogProps) {
  ...
  // Mock inventory items - replace with actual API call
  const mockInventoryItems: InventoryItem[] = [
    { 
      id: "inv1", 
      nombre: "Champú Premium", 
      descripcion: "Champú premium para vehículos",
      stockActual: 50, 
      stockMinimo: 10,
      unidadMedida: "UN",
      proveedor: null,
    ...
    { 
      id: "inv2", 
      nombre: "Cera Líquida", 
      descripcion: "Cera protectora líquida",
      stockActual: 30, 
      stockMinimo: 5,
      unidadMedida: "UN",
      proveedor: null,
    ...
    { 
      id: "inv3", 
      nombre: "Ambientador", 
      descripcion: "Ambientador para vehículos",
      stockActual: 100, 
      stockMinimo: 20,
      unidadMedida: "UN",
      proveedor: null,
  ...
  // Initialize items from work order services
  useEffect(() => {
    if (workOrder && services.length > 0 && isOpen) {
      // Get services from work order - for now using mock data
      const workOrderServices = services.slice(0, 2).map(service => ({
        type: 'service' as const,
        id: service.id,
        name: service.nombre,
        price: parseInt(service.precio),
        quantity: 1,
        total: parseInt(service.precio),
      }));
      setItems(workOrderServices);
    }
  }, [workOrder, services, isOpen]);
  ...
  const createSaleMutation = useMutation({
    mutationFn: async (data: { saleData: SaleFormData; action: 'save' | 'save-print' | 'print' }) => {
      const response = await apiRequest('POST', '/api/sales/create-from-order', data);
  ...
  const filteredProducts = mockInventoryItems.filter(item =>
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Punto de Venta - Orden #{workOrder.numero}
          </DialogTitle>
          <DialogDescription>
